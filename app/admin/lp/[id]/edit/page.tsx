...
export default function LpEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/lp/${id}/update`)
      ...
      .then((data) => {
        ...
        if (data.template) {
          setSections(parseTemplateSections(data.template.sections));
        }
        setContentData(parseContentData(data.contentData));
      });
  }, [id, router]);

  // 構造化モードの場合、contentDataからHTMLをリアルタイム生成
  ...
  const handleSubmitPreview = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        toast.error(data.error || "提出に失敗しました");
        return;
      }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 border-b">
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-2 text-sm border-b-2 transition-colors ${mode === "structured" ? "border-blue-600 text-blue-600 font-medium" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          <FormInput className="w-4 h-4" /> 構造化エディタ
        </button>
        <button
          onClick={() => {
            if (mode === "structured") {
          ...
          className={`flex items-center gap-1.5 px-4 py-2 text-sm border-b-2 transition-colors ${mode === "code" ? "border-blue-600 text-blue-600 font-medium" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          <Code className="w-4 h-4" /> コードエディタ
        </button>
      </div>
